...
STARTS (TIMESTAMP(NEXT_DAY(CURRENT_DATE, 'MONDAY')) + INTERVAL 7 HOUR)
ON COMPLETION PRESERVE
ENABLE
...
-- Schedule: First day of every month at 08:00
...
STARTS (DATE_FORMAT(NOW(), '%Y-%m-01 08:00:00') + INTERVAL 1 MONTH)
ON COMPLETION PRESERVE
ENABLE
...
-- Schedule: 1st of every month at 05:00 (before business hours)
...
ON SCHEDULE EVERY 1 MONTH
STARTS (DATE_FORMAT(NOW(), '%Y-%m-01 05:00:00') + INTERVAL 1 MONTH)
ON COMPLETION PRESERVE
ENABLE
...
WHERE routine_schema = 'group19GA'
...
WHERE trigger_schema = 'group19GA'
