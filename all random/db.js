  ...
  connect: async () => {
    try {
      mysqlPool = mysql.createPool({
        ...
        database:           process.env.MYSQL_DATABASE || "jhb_imts",
...
const mongo_db = {
  ...
  connect: async () => {
    try {
      ...
      mongoDb = mongoClient.db(process.env.MONGO_DATABASE || "jhb_imts_mongo");

      // Ping to verify
      await mongoDb.command({ ping: 1 });
      console.log("[MongoDB] Connected — database:", process.env.MONGO_DATABASE || "jhb_imts_mongo");
    } catch (err) {
